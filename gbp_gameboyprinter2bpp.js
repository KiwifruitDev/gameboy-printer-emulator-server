...
Modified for node.js compatibility by KiwifruitDev
...
function renderImage(tiles, palette = "grayscale")
{
    ...
    canvasWidth = square_width * TILE_PIXEL_WIDTH * TILES_PER_LINE;
    canvasHeight = square_height * TILE_PIXEL_HEIGHT * tile_height_count;
    ...
    tiles.forEach(function (pixels, index)
    {
        ...
        paint(canvas, pixels, square_width, square_height, tile_x_offset, tile_y_offset, palette);
    })

    return canvas;
}

function render_gbp(rawBytes, palette = "grayscale")
{
    ...
    {
        canvases.push(renderImage(image, palette));
    });
...
function paint(canvas, pixels, pixel_width, pixel_height, tile_x_offset, tile_y_offset, palette = "grayscale")
{
    switch (palette)
    {
        case "grayscale":
